...
from creversi_gym.player.human_player import HumanPlayer, RESIGN, QUIT, UNDO
...
def main(player1, player2, model1='model.pt', model2=None, network1='dqn', network2='dqn', temperature=0.1, games=1, is_display=True):
    ...
    for n in range(games):
        ...
        while not board.is_game_over():
            bd_log[i] = board.to_line()
            i += 1

            if is_display:
                print(f'\n{i}: ' + ('black' if board.turn == BLACK_TURN else 'white'))
                if is_jupyter:
            ...
            if board.puttable_num() == 0:
                ...
                if isinstance(player, HumanPlayer):
                    if move == RESIGN:
                        ...
                        if i > 2:
                            i -= 3
                            board = Board(bd_log[i], board.turn == BLACK_TURN)     # 前の自分の番(2個前)に戻す
                            continue
                        else:
                            print('can\'t execute UNDO')
                            i -= 1
                            continue
